...
3. Nhân viên dán key, setup link hoặc lệnh triển khai do admin gửi.
4. Extension tải danh sách email theo key, nhân viên chỉ cần chọn email rồi phòng ban tự điền.
5. Nhân viên bấm **Đăng ký và bật bảo vệ**.
...
```powershell
.\aiguard-extension setup --api https://api.company.vn --token "<enrollment-key>" --email "employee@company.com"
```

Hoặc setup link:

```text
chrome-extension://<extension-id>/options.html?api=https%3A%2F%2Fapi.company.vn&token=<key>&email=employee@company.com
```
...
2. Enrollment key.
3. Chọn email trong danh sách.
4. Phòng ban sẽ tự điền.
