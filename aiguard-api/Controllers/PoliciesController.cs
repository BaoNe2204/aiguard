...
public class PoliciesController : ControllerBase
{
    ...
    {
        var result = await _policyService.GetVersionsAsync();
        return Ok(ApiResponse<List<PolicyVersionResponse>>.Ok(result));
    }

    /// <summary>Rollback policy to default values</summary>
    [HttpPost("versions/{id:guid}/rollback")]
    public async Task<IActionResult> Rollback(Guid id)
    {
        var ok = await _policyService.RollbackAsync(id);
        if (!ok) return NotFound(ApiResponse<object>.Fail("Policy not found"));
        return Ok(ApiResponse<object>.Ok(new { }, "Policy rolled back to defaults"));
    }
